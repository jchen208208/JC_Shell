...
./node_modules/.bin/electron rotom-gen.js` → overwrites `gui/rotom.png`). Every
feature is a parameterised shape — horn length, arm size, eye tilt, screen rect
are all numbers, so "make the arms 2px longer" is a one-line change. The 232x160
canvas is 3x finer than the first 80x64 art, which is what "less pixelated but
still pixel art" means concretely: 4 CSS px per art px at scale 4 instead of 12.
The screen area is filled with the terminal's own background colour (`#1e1e2e`),
so a 1px alignment error between art and terminal is invisible.

History, kept because the pipeline may be useful again: the first art came from
a generator site as `rotomdex_window_1.png` (still in `gui/`), which *looked*
like pixel art but was 1024x825 with 30,336 unique colours, a fractional
12.8 x 12.89 px block size and 13,452 semi-transparent pixels — compression
noise that `image-rendering: pixelated` would have magnified. It was rescued by
fitting the pixel grid (difference-profile scoring: columns peaked at 80, rows
at 64), resampling one modal colour per cell from the middle ~44% of each cell,
and merging into a 10-colour palette. That 80x64 art's screen was only 28x14
cells (38 cols x 10 rows in a 960x768 window), which killed it — hence the
redraw, per direction: arms ~1/10, horns ~30%, legs ~20%, screen maximised.

**Geometry** (all in `gui/layout.js`, art pixels): art 232x160, screen
27,43 178x101, buttons on the two face dots. Screen is 77% of the art's width
and 63% of its height. At scale 4 that is exactly **80 cols x 24 rows** at font
14 (measured xterm.js cell: 8.84 x 16.80 px).

**Scale is computed at launch, not hardcoded:** the biggest whole number where
`art * scale` fits the primary display's work area. 1080p display → scale 6 →
1392x960 window with a 120x36 terminal; the 800x600 test display → scale 3 →
696x480 with 61x18. The window can never exceed the screen it opens on.

Verified end to end on the redrawn art: synthesized real mouse clicks on the two
dot buttons — fullscreen on, fullscreen off (size restored exactly), close fires
`closed`. The earlier finding stands: in fullscreen the frame stretches, so
anything computing button positions must derive them from the *current* window
size, as `applyLayout()` does.
